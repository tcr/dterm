...
"e, o, n, m, s	Set parity"	
#ifdef CCTS_OFLOW
"		cts,nocts	Enable / disable CTS\n"
#else
"\n"
#endif
...
main(int argc, char **argv) {
	...
	/*
	 ...
	 Look for the first on-board device first; failing that look for
	 the first USB device.  For Linux & BSD these are:
		OS	Onboard	USB
		Linux	ttyS0	ttyUSB0
		BSD	ttyd0	ttyU0
	 Note that this rather assumes that the existence of a device
	 name in /dev indicates the actual existence of a device.  Mostly
	 this is the case on modern systems.  Mostly.
	 */
	if(!device) {
		if(     !stat("/dev/ttyS0",   &sb))  device = "/dev/ttyS0";
		else if(!stat("/dev/ttyd0",   &sb))  device = "/dev/ttyd0";
		else if(!stat("/dev/ttyUSB0", &sb))  device = "/dev/ttyUSB0";
		else if(!stat("/dev/ttyU0",   &sb))  device = "/dev/ttyU0";
		else {
